...
def load_rgb(path, mean_rgb, std_rgb):
    ...
    mean_rgb = np.array(mean_rgb).reshape(1, 1, 3)
    std_rgb = np.array(std_rgb).reshape(1, 1, 3)
...
def idr_get_sphere_intersection(cam_loc, ray_directions, r = 1.0):
...
def get_sphere_intersection(origin_points, ray_directions, r = 1.0):
    # Input: n_images x 4 x 4 ; n_images x n_rays x 3
    # Output: n_images * n_rays x 2 (close and far) ; n_images * n_rays

    n_imgs, n_pix, _ = ray_directions.shape

    ray_cam_dot = torch.sum(ray_directions * origin_points, dim=2).squeeze()
    under_sqrt = ray_cam_dot ** 2 - (origin_points.norm(2, 2) ** 2 - r ** 2)
