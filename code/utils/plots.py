...
def plot(model, indices, model_outputs ,pose, rgb_gt, path, epoch, img_res, plot_nimgs, max_depth, resolution, is_train_vertices):
    ...
    # depth = torch.zeros(batch_size * num_samples).cuda().float() * max_depth
    # depth[network_object_mask] = 1.0
    # depth = depth.reshape(batch_size, num_samples, 1)
    # network_object_mask = network_object_mask.reshape(batch_size,-1)
...
def get_surface_trace(path, epoch, sdf, resolution=100, return_mesh=False):
    ...
    if (not (np.min(z) > 0 or np.max(z) < 0)):
        ...
        torch.cuda.synchronize()
        # end = time.time()
        # print("marching cube time: {0} s".format(end - start))
        # input()
        # fixed_verts, fixed_faces = fix_mesh_raw(meshexport.vertices, meshexport.faces)
        # meshexport = trimesh.Trimesh(fixed_verts, fixed_faces)
...
def get_grid_uniform(resolution):
    ...
    # x = np.linspace(-0.56, 0.56, resolution)
    # x = np.linspace(-0.45, 0.45, resolution)
    x = np.linspace(-0.65, 0.65, resolution) # test hand
    y = x
    z = x
...
def plot_images(rgb_points, ground_true, path, epoch, plot_nrow, img_res):
    ...
    # mean_rgb = torch.from_numpy(np.array(mean_rgb)).view(1, 3).float().cuda()
    # std_rgb = torch.from_numpy(np.array(std_rgb)).view(1, 3).float().cuda()
    # # ground_true = ground_true.cuda() * std_rgb + mean_rgb
    # rgb_points = rgb_points.cuda() * std_rgb + mean_rgb
    ground_true = ground_true.cuda() / 255.0
    ...
    rgb_points = lin2img(rgb_points, img_res).squeeze()
    rgb_points = rgb_points.permute(1, 2, 0)
    ground_true = lin2img(ground_true, img_res).squeeze()
    ground_true = ground_true.permute(1, 2, 0)
    diff_image = torch.abs(rgb_points - ground_true)
    
    rgb_points = (rgb_points * scale_factor).cpu().detach().numpy().astype(np.uint8)
    ground_true = (ground_true * scale_factor).cpu().detach().numpy().astype(np.uint8)
    diff_image = (diff_image * scale_factor).cpu().detach().numpy().astype(np.uint8)
    img = Image.fromarray(rgb_points)
    img.save('{0}/{1}_rendering.png'.format(path, epoch))
    img = Image.fromarray(ground_true)
    img.save('{0}/{1}_gt.png'.format(path, epoch))
    img = Image.fromarray(diff_image)
    img.save('{0}/{1}_diff.png'.format(path, epoch))
